import ogImage from 'virtual:og-image?title=Performance Optimization&description=Achieving near-perfect Lighthouse scores with React Compiler, route-level code splitting, build-time module preloading, and edge caching.&tags=Performance,React,Vite&type=blog&seriesPart=8&seriesTotalParts=9';
...
A portfolio with a Lighthouse score of 60 tells recruiters one thing: this person doesn't practice what they preach. Performance optimization is where frameworks usually earn their keep—Next.js handles code splitting, image optimization, and preloading automatically. Without a framework, you handle it yourself.

The good news: the primitives aren't complicated. The bad news: nobody's going to remind you when you forget one.
...
These numbers are achievable because the site is fundamentally simple: pre-rendered HTML, minimal JavaScript, fast hosting. No amount of optimization saves a poorly architected site. The real performance work happened in Parts [3](/blog/building-portfolio-part-3-ssg) and [5](/blog/building-portfolio-part-5-hosting)—SSG and edge hosting are the foundation. Everything in this post is polish on top of those decisions.
