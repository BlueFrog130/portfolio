import ogImage from 'virtual:og-image?title=Building a Custom React Router&description=How to build a custom React router from scratch with file-based route discovery, pattern matching, lazy loading, and the ViewTransition API.&tags=React,Router,ViewTransitions&type=blog&seriesPart=2&seriesTotalParts=9';
...
Before diving into the router itself, here's how routes are defined. Instead of manually maintaining an array of route objects, I use Vite's `import.meta.glob` to auto-discover pages from the filesystem—heavily inspired by SvelteKit's routing conventions:
...
<Tree
	...
	items={[
		...
		{ name: '+Page.options.ts', description: 'Route options for /' },
		{
			name: '404',
			children: [{ name: '+Page.tsx', description: '* (catch-all)' }],
		},
		{
			name: 'blog',
			children: [
				{ name: '+Page.tsx', description: '/blog' },
				{
					name: '[slug]',
					children: [
						{ name: '+Page.tsx', description: '/blog/:slug' },
						{ name: '+Page.options.ts' },
					],
				},
			],
		},
		{
			...
			children: [
				{
					name: '[slug]',
					children: [{ name: '+Page.tsx', description: '/project/:slug' }],
				},
			],
		},
		{
			name: 'terminal',
			children: [{ name: '+Page.tsx', description: '/terminal' }],
		},
	]}
/>
...
- **Dynamic routes use `[param]` folders**: Similar to Next.js/SvelteKit convention.
- **`entries()` for dynamic routes**: Returns all possible paths for SSG pre-rendering.
- **Catch-all with `/404/`**: The 404 folder is special-cased to the `*` pattern.
...
- Dynamic routes: `/project/[slug]/` matches `/project/portfolio` with `{ slug: 'portfolio' }`
- Catch-all: `*` matches anything (mapped from the `/404/` folder)
	...
	items={[
		{
			name: 'assets',
			children: [
				{ name: 'index-[hash].js', description: 'Core app (~50KB)' },
				{ name: '+Page-[hash].js', description: 'Home page' },
				{ name: '+Page-[hash].js', description: 'Blog list' },
				{ name: '+Page-[hash].js', description: 'Blog post' },
				{ name: '+Page-[hash].js', description: 'Project page' },
				{ name: '+Page-[hash].js', description: 'Terminal' },
			],
		},
	]}
/>
...
That's it. No build plugins, no babel transforms, no special file naming conventions beyond folder structure. Just React components and the History API.
