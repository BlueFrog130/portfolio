import ogImage from 'virtual:og-image?title=Hosting on Cloudflare Workers&description=Deploying a portfolio on Cloudflare Workers with Hono, D1 database, Workers AI, and the Vite Cloudflare plugin for local development.&tags=Cloudflare,Hono,Edge&type=blog&seriesPart=5&seriesTotalParts=9';
...
My portfolio costs $0/month to host, runs in 300+ data centers, and has a SQLite database and LLM inference built in. That's not a sales pitch—it's what Cloudflare Workers gives you on the free tier.

I didn't choose Workers because it's the easiest option. I chose it because it lets me run real backend code at the edge with tight integration between compute, storage, and AI—all from a single vendor.
...
D1 is Cloudflare's serverless SQLite database. It's not a traditional database server—each request gets a connection to a SQLite file replicated globally. No connection pools, no cold-start latency for database connections, no separate infrastructure to manage.

I'll cover the D1 schema and queries in detail in [Part 7](/blog/building-portfolio-part-7-analytics), where we build the analytics system. For now, the key thing to know: D1 bindings work exactly like AI bindings—typed objects on `c.env`, available instantly.
...
That's it. No Docker, no Kubernetes, no infrastructure-as-code. Just a TypeScript file and a config. The infrastructure disappears so you can focus on what it enables.
