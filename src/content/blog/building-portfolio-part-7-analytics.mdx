import ogImage from 'virtual:og-image?title=Analytics from Scratch&description=Privacy-respecting analytics with session tracking, Web Vitals, and Grafana dashboards.&tags=Analytics,Privacy,Grafana&type=blog&seriesPart=7&seriesTotalParts=10';
...
When someone arrives, capture contextual information:
...
Track which pages get visited and how long people spend:
...
Google's Core Web Vitals measure real-world performance. The `web-vitals` library makes collection trivial:
...
For anything else—chat interactions, button clicks, feature usage:
...
```typescript
client.trackEvent('chat_opened', 'chat', { projectSlug: 'portfolio' });
client.trackEvent('stream_completed', 'chat', {
	responseLength: 450,
	streamDurationMs: 2100,
});
```

Flexible enough to track anything without schema changes.
...
class AnalyticsClient {
	...
	private startFlushTimer(): void {
		this.flushTimer = window.setInterval(() => {
			this.flush();
		}, this.config.flushInterval); // Default: 5 seconds
	}
}
```
...
When the user switches tabs or closes the page, any queued events get sent immediately.
